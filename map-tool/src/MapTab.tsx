...
function MapTab({
  ...
  const handleMapMouseMove = (e: MouseEvent) => {
    ...
    if (deleteZoneRef.current) {
      ...
      const d = e.clientX >= deleteRect.left && e.clientX <= deleteRect.right &&
               e.clientY >= deleteRect.top && e.clientY <= deleteRect.bottom;
      if (d !== isInDeleteZone) setIsInDeleteZone(d);
    }

    const stageRect = mapStageRef.current.getBoundingClientRect();
    dragPosRef.current = {
      x: Math.max(0, Math.min(100, ((e.clientX - stageRect.left - markerOffset.x) / stageRect.width) * 100)),
      y: Math.max(0, Math.min(100, ((e.clientY - stageRect.top - markerOffset.y) / stageRect.height) * 100)),
    };

    setHasMoved(true);

    const markerEl = mapStageRef.current.querySelector(`[data-marker-id="${draggingMarkerId}"]`) as HTMLElement;
    if (markerEl) {
      markerEl.style.left = `${dragPosRef.current.x}%`;
      markerEl.style.top = `${dragPosRef.current.y}%`;
    }
  };
    ...
    if (draggingMarkerId) {
      if (isInDeleteZone) {
        ...
        setCurrentMapMarkers(markers =>
          markers.map(m =>
            m.id === draggingMarkerId ? { ...m, x: dragPosRef.current.x, y: dragPosRef.current.y } : m
          )
        );
      }
